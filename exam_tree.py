...
class MCTS:
    ...
    def dispatch_and_retrieve_jobs(self, server = None):
        while len(self.TASK_QUEUE) > 0:
            ...
            try:
                print("\ndispatch -")
                print("number of networks in training:", self.JOB_COUNTER)
                ...
                self.TOTAL_SEND += 1
                self.JOB_COUNTER += 1
                print("dispatch job, total_send/total_recv: {}/{}".format(self.TOTAL_SEND, self.TOTAL_RECV))
                ...
                for i in range(0, 5):
                    ...
                    if conn.poll(3):
                        ...
                        maeinv                       = receive_signal[2]
                        best_model                   = receive_signal[3]
                        report                       = receive_signal[4]
                        self.DISPATCHED_JOB[job_str] = maeinv
                        self.samples[job_str]        = maeinv
                        self.mae_list.append(1 / maeinv)
                        self.TOTAL_RECV += 1
                        self.JOB_COUNTER -= 1
                        print("retrieve job from {}: {}".format(client_name, job_str))
                        ...
                        print("current number of samples: {}, job counter: {}".format(len(self.samples), self.JOB_COUNTER))
                        print("total_send/total_recv: {}/{}".format(self.TOTAL_SEND, self.TOTAL_RECV))
    ...
    def search(self):
        address = ('10.188.218.70', 8000)
        server = Listener(address, authkey=b'qfnet')

        while len(self.search_space) > 0:
            self.dump_all_states(len(self.samples))
            print("\niteration:", self.ITERATION)

            # dispatch and retrieve jobs:
            print("\ndispatch and retrieve jobs...")
